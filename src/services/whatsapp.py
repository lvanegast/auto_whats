...
class OpenWAClient:
    ...
    async def check_status(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                ...
                logger.error(
                    f"Error de estado HTTP al verificar sesión: {e.response.status_code} - {e.response.text}"
                )
                raise e
    ...
    async def send_text_message(self, chat_id: str, text: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            try:
                ...
                logger.error(
                    f"Error de OpenWA al enviar mensaje: {e.response.status_code} - {e.response.text}"
                )
                raise e
            except httpx.RequestError as e:
    ...
    async def send_buttons(
        self, chat_id: str, text: str, buttons: list[dict]
    ) -> Dict[str, Any]:
        """
        ...
        payload = {"chatId": chat_id, "text": text, "buttons": buttons}
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            try:
                ...
                logger.error(
                    f"Error de OpenWA al enviar botones: {e.response.status_code} - {e.response.text}"
                )
                raise e
            except httpx.RequestError as e:
...
openwa_client = OpenWAClient(
    base_url=OPENWA_API_URL, api_key=OPENWA_API_KEY, session_id=OPENWA_SESSION_ID
)
